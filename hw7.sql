...
INNER JOIN catalogs ON catalog_id = catalogs.id;
...
DROP TABLE IF EXISTS fligts;
CREATE TABLE fligts(
	id SERIAL PRIMARY KEY,
	from_city VARCHAR(255) COMMENT 'City from',
	to_city VARCHAR(255) COMMENT 'City to'
)

INSERT INTO fligts (from_city, to_city)
VALUES 
('moscow', 'omsk'),
('novgorod', 'kazan'),
('irkutsk', 'moscow'),
...
CREATE TABLE cities(
	id SERIAL PRIMARY KEY,
	label VARCHAR(255) UNIQUE COMMENT 'English label',
...
SELECT 
	id,
	(SELECT name FROM cities WHERE label = from_city),
	(SELECT name FROM cities WHERE label = to_city)
FROM fligts;
