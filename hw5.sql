...
UPDATE users 
	SET created_at = '22.12.2021 11:52',
	updated_at ='22.12.2021 11:52';
 ...
 * Месяцы заданы в виде списка английских названий 
 */
...
SELECT ROUND(AVG((TO_DAYS(NOW()) - TO_DAYS(birthday_at)) / 365.25)) FROM users;
...
SELECT COUNT(*), DAYNAME(DATE_ADD(birthday_at, INTERVAL FLOOR((TO_DAYS(NOW()) - TO_DAYS(birthday_at)) / 365.25) YEAR)) AS day_of_week FROM users GROUP BY day_of_week;

/*
 * Задача 2.3
 * Подсчитайте произведение чисел в столбце таблицы.
 * 
 * P.S решение загуглила, но считаю, что нужно уметь пользоваться гуглом )
 */

UPDATE storehouses_products 
	SET value = 2 WHERE value = 0;
