...
DROP TABLE IF EXISTS playlists_audios;
CREATE TABLE playlists_audios(
	audio_playlist_id BIGINT UNSIGNED NOT NULL,
	audio_id BIGINT UNSIGNED NOT NULL,
  
	PRIMARY KEY (audio_playlist_id, audio_id), 
    FOREIGN KEY (audio_playlist_id) REFERENCES audio_playlist(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (audio_id) REFERENCES audios(id) ON UPDATE CASCADE ON DELETE CASCADE
);
